                  ...
                  <div class="form-row">
                    ...
                    <?php
                     if(!empty($parents) && count($parents) > 0){
                      ?>
                      <div class="col-md-6">
                       <label for="fullname" class="form-label">{{$parent_role->name??''}}</label>
                       <select class="form-control select2" name="parent_id">
                        <option value="" selected disabled>Select {{$parent_role->name??''}}</option>
                        <?php if(!empty($parents)  && count($parents) > 0){
                          foreach($parents as $parent){
                            ...
                            ?>
                            <option value="{{$parent->id}}" <?php if($parent->id == $parent_id) echo "selected"?>>{{$business_name_new??''}} -  {{$parent->unique_id}}</option>
                          <?php }}else{?>
                   ...
                   <div class="col-md-6">
                     ...
                     <input class="form-control mb-3" type="text" name="address" id="address"  value="{{old('address',$address)}}" placeholder="Business Address" aria-label="default input example">
                   </div>

                   <div class="col-md-6">
                     <label for="fullname" class="form-label">Owner Name</label>
                     <input class="form-control mb-3" type="text" name="name" id="name"  value="{{old('name',$name)}}" placeholder="Name" aria-label="default input example" placeholder="Owner Name">
                   </div>
                   <div class="col-md-6">
                    <label for="email" class="form-label">Email</label>
                    <input class="form-control mb-3" type="email" name="email" id="email"  value="{{old('email',$email)}}" placeholder="Email" aria-label="default input example" placeholder="Email">
                  </div>

                  
                  <div class="col-md-6">
                    <label for="fullname" class="form-label">Phone</label>
                    <input class="form-control mb-3" type="number" name="phone" value="{{old('phone',$phone)}}" id="phone"  aria-label="default input example" placeholder="Phone">
                  </div>
                  <div class="col-md-6">
                    <label for="fullname" class="form-label">Alternate Phone</label>
                    <input class="form-control mb-3" type="number" name="alternate_phone" value="{{old('alternate_phone',$alternate_phone)}}" placeholder="Alternate Phone" id="alternate_phone"  aria-label="default input example">
                  </div>


                  <div class="col-md-6">
                    <label for="fullname" class="form-label">Username</label>
                    <input class="form-control mb-3" type="text" name="username" value="{{old('username',$username)}}" id="username"  aria-label="default input example" placeholder="Username">
                  </div>
                    ...
                    <select class="form-control" name="country_id" id="country_id">
                      ...
                      <?php if(!empty($countries)){
                        foreach($countries as $coun){
                          ?>
                          <option value="{{$coun->id}}" <?php if($coun->id == $country_id) echo "selected"?>>{{$coun->name??''}}</option>
                        <?php }}?>
                      </select>
                    </div>
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <div class="col-md-6 mb-3">
                      ...
                      <div class="col-md-6">
                        ...
                        <input class="form-control mb-3" type="text" name="gst" value="{{old('gst',$gst)}}" id="gst"  aria-label="default input example" placeholder="GST No">
                      </div>

                      <div class="col-md-6">
                        <label for="fullname" class="form-label">Locality (Pincode)</label>
                        <input class="form-control mb-3" type="text" name="pincode" value="{{old('pincode',$pincode)}}" id="pincode" placeholder="Pincode"  aria-label="default input example">
                      </div>
