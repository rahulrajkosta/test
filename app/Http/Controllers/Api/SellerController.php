...

{
    ...
    public function check_phone_status(Request $request)
    {
        ...
        $rsp=$this->get_devices($request->serial_no,$request->imei_no);
		//Rahul///
		if($rsp['status']==400){
			return response()->json([
			'result' => false,
			'message' => "Invalid IMEI no.",
			], 200);
		}
		//end//
        $data = null;
    ...
    public function get_devices($serial_no,$imei_no){
       ...
       // $response = implode(",", $response);
		...
		$return =MobileDetails::where('imei_no',$imei_no)->update(['device_id'=>$response]);
		//echo $return;
		if($return) 
		{
			return $dta=['status'=>200];
		}else{
			return $dta=['status'=>400];
		}
		//end//
