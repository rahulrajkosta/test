...
Class InvoiceController extends Controller
{
   ...
   public function add(Request $request){
    ...
    if(is_numeric($id) && $id > 0){
        ...
        if(empty($invoice)){
            return redirect($this->ADMIN_ROUTE_NAME.'/invoice');
        }
    }
    ...
    if($request->method() == 'POST' || $request->method() == 'post'){
        ...
        if(empty($back_url)){
            $back_url = $this->ADMIN_ROUTE_NAME.'/invoice';
        }
    ...
    return view('admin.invoice.form', $data);
...
public function change_admin_state(Request $request){
    ...
    if($method == 'post' || $method == 'POST'){
        ...
        Admin::where('id',$request->admin_id)->update(['states'=>$states]);

        return back()->with('alert-success', 'State Updated successfully.');
...
public function change_admin_superdist(Request $request){
    ...
    if($method == 'post' || $method == 'POST'){
        $rules = [];
        $rules['admin_id'] = 'required';
        $rules['superdistributors'] = 'required';

        $this->validate($request, $rules);


        $superdistributors = implode(",", $request->superdistributors);

        Admin::where('id',$request->admin_id)->update(['superdist_id'=>$superdistributors]);

        return back()->with('alert-success', 'Superdistributors Updated successfully.');
