...
class CustomHelper{
    ...
    public static function getFacultyRouteName(){
        $FACULTY_ROUTE_NAME = config('custom.FACULTY_ROUTE_NAME');

        if(empty($FACULTY_ROUTE_NAME)){
            $FACULTY_ROUTE_NAME = 'faculty';
        }

        return $FACULTY_ROUTE_NAME;
    }

    public static function getParentCoupons($parent_id){
        $coupons = [];
        if($parent_id != 0){
            $coupon_count = Admin::where('id',$parent_id)->first();
            if($coupon_count->no_of_coupons > 0){
                $parent_total_coupons = AssignCoupon::select('coupon_id')->where('is_return',0)->where('child_id',$parent_id)->latest()->pluck('coupon_id')->toArray();
                $distribute_coupons = AssignCoupon::select('coupon_id')->where('is_return',0)->where('parent_id',$parent_id)->latest()->pluck('coupon_id')->toArray();
                $couponArr = [];
                ...
                //         $couponArr[$key]=$value;
                // }
                $couponArr = array_diff($parent_total_coupons, $distribute_coupons);
                // prd($couponArr);
                // $couponArr = array_reverse($couponArr);
                // prd($couponArr);
                $coupons = Coupon::select('id','couponID')->whereIn('id',$couponArr)->get();
    ...
    public static function getParentCouponsNew($parent_id){
        $coupons = [];
        if($parent_id != 0){
            $coupon_count = Admin::where('id',$parent_id)->first();
            if($coupon_count->no_of_coupons > 0){
                // $parent_total_coupons = AssignCoupon::select('coupon_id')->where('is_return',0)->where('child_id',$parent_id)->pluck('coupon_id')->toArray();
                $distribute_coupons = AssignCoupon::select('coupon_id')->where('is_return',0)->where('parent_id',$parent_id)->pluck('coupon_id')->toArray();
                ...
                // }
                // \DB::enableQueryLog();
                $coupons = AssignCoupon::select('coupons.id','coupons.couponID')->rightJoin('coupons', 'coupons.id', '=', 'assign_coupons.coupon_id')->where('assign_coupons.child_id',$parent_id)->orWhere('assign_coupons.parent_id','!=',$parent_id)->get();
                // dd(\DB::getQueryLog());
                // $coupons = Coupon::select('id','couponID')->whereIn('id',$couponArr)->get();
                // $coupons = Coupon::select('id','couponID')->whereIn('id',$couponArr)
                // ->chunk(50, function($coupons) {
...
// public static function getOwnCoupons($user_id){
//         $coupons = [];
//         if($parent_id != 0){
...
//                     $couponArr[$key]=$value;
//             }
//             $coupons = Coupon::whereIn('id',$couponArr)->get();
//         }
//     }
//     echo  count($coupons);
//     die;
// }
    ...
    public static function getParents($parent_id){
        ...
        $users = [];
        $parent = Admin::select('role_id')->where('id',$parent_id)->first();
        
        if(!empty($parent)){
            $parent_role = Roles::where('id',$parent->role_id)->first();
            if(!empty($parent_role)){
                $users = Admin::select('id','business_name','name','unique_id')->where('role_id',$parent->role_id)->get();
            } 
        }
    ...
    public static function getMyCouponCount($user_id){
       $parent_total_couponsall = \App\AssignCoupon::where('child_id',$user_id)->where('is_return',0)->groupBy('coupon_id')->pluck('coupon_id')->toArray();
       if($user_id == 40){

     // prd($parent_total_couponsall);
       }
       $distribute_couponsall = \App\AssignCoupon::where('parent_id',$user_id)->where('is_return',0)->pluck('coupon_id')->toArray();
       $couponArr = [];
       if(!empty($parent_total_couponsall)){
        foreach ($parent_total_couponsall as $key => $value){
          if(!in_array($value, $distribute_couponsall))
            $couponArr[$key]=$value;
    }
    if($user_id == 40){
        // AssignCoupon::where('child_id',40)->whereIn('coupon_id',$couponArr)->update(['is_return'=>1]);
        // prd($couponArr);
    }
...
public static function returnCouponBalanace($user_id,$is_view=0){
    $parent_total_coupons = AssignCoupon::where('is_return',0)->where('is_used',0)->where('child_id',$user_id);
    if($is_view == 1){
        $parent_total_coupons->where('is_view',0);
    }
    $parent_total_coupons = $parent_total_coupons->pluck('coupon_id')->toArray();
    $distribute_coupons = AssignCoupon::where('is_return',0)->where('is_used',0)->where('parent_id',$user_id);
    if($is_view == 1){
      $distribute_coupons->where('is_view',0);
  }
  $distribute_coupons = $distribute_coupons->pluck('coupon_id')->toArray();
...
public static function getMyUsedCoupon($user_id,$role_id){
    $coupons = 0;
    if($user_id != 0){
        if($role_id == 5){
            $coupons = AssignCoupon::where('is_used',0)->where('is_return',0)->where('child_id',$user_id)->where('is_used',1)->count();
        }
        if($role_id == 4){
            $parent_ids = Admin::where('parent_id',$user_id)->where('role_id',5)->pluck('id')->toArray();
            $coupons = AssignCoupon::whereIn('id',$parent_ids)->where('is_return',0)->where('child_id',$user_id)->where('is_used',1)->count();
        }
        if($role_id == 3){
            $parent_ids = Admin::where('parent_id',$user_id)->where('role_id',4)->pluck('id')->toArray();
            $child_ids = Admin::whereIn('parent_id',$parent_ids)->where('role_id',5)->pluck('id')->toArray();
            $coupons = AssignCoupon::whereIn('id',$parent_ids)->where('is_return',0)->where('child_id',$user_id)->where('is_used',1)->count();
        }

        if($role_id == 2){
            $parent_ids = Admin::where('parent_id',$user_id)->where('role_id',3)->pluck('id')->toArray();
            $child_ids = Admin::whereIn('parent_id',$parent_ids)->where('role_id',4)->pluck('id')->toArray();
            $child_ids1 = Admin::whereIn('parent_id',$child_ids)->where('role_id',5)->pluck('id')->toArray();
            $coupons = AssignCoupon::whereIn('id',$child_ids1)->where('is_return',0)->where('child_id',$user_id)->where('is_used',1)->count();
        }
    }
    return $coupons;
}

public static function getCountChild($role_id,$user_id,$i){
    ...
    if($i == 1){
        $all_user = Admin::where('role_id',$role_id)->where('status',1)->where('is_delete',0)->where('parent_id',$user_id)->get();
        $count = count($all_user);
    }
    if($i == 2){
        $parent_users = Admin::where('parent_id',$user_id)->where('status',1)->where('is_delete',0)->get();
        if(!empty($parent_users)){
            foreach($parent_users as $all){
                $parent_ids[] = $all->id;
            }
        }
        if(!empty($parent_ids)){
            $child_user = Admin::whereIn('parent_id',$parent_ids)->where('status',1)->where('is_delete',0)->count();
            $count = $child_user;
        }
    }
    if($i == 3){
        ...
        if(!empty($parent_ids)){
            $child_user = Admin::whereIn('parent_id',$parent_ids)->where('status',1)->where('is_delete',0)->get();
            if(!empty($child_user)){
                foreach($child_user as $child){
                    $child_ids[] = $child->id;
           ...
           // print_r($child_ids);
            if(!empty($child_ids)){
                $child_user = Admin::whereIn('parent_id',$child_ids)->where('status',1)->where('is_delete',0)->count();
                $count = $child_user;
            }
        }
    }
    if($i == 4){
        ...
        if(!empty($parent_ids)){
            ...
            if(!empty($child_ids)){
                $child_user = Admin::whereIn('parent_id',$child_ids)->where('status',1)->where('is_delete',0)->get();
                if(!empty($child_user)){
                    foreach($child_user as $chi){
                        $subchild_ids[] = $chi->id;
                    }
                }
                if(!empty($subchild_ids)){
                    $child_user = Admin::whereIn('parent_id',$subchild_ids)->where('status',1)->where('is_delete',0)->count();
                    // prd($child_user);
                    $count = $child_user;
                }
            }
...
public static function getChildIds($role_id,$user_id,$i,$key=''){
    $user = Admin::where('id',$user_id)->first();
    $role = Roles::where('id',$role_id)->first();
    $count = 0;
    $parent_ids = [];
    ...
    if($i == 1){
        $all_user = Admin::where('role_id',$role_id)->where('parent_id',$user_id)->get();
        if(!empty($all_user)){
            $ids = [];
    ...
    if($i == 2){
        $parent_users = Admin::where('parent_id',$user_id)->get();
        if(!empty($parent_users)){
            $ids = [];
            foreach($parent_users as $all){
                $ids[] = $all->id;
    ...
    if($i == 3){
        ...
        if(!empty($parent_ids)){
            $child_user = Admin::whereIn('parent_id',$parent_ids)->get();
            if(!empty($child_user)){
                $ids = [];
                foreach($child_user as $child){
                    $ids[] = $child->id;
    ...
    if($i == 4){
        ...
        if(!empty($parent_ids)){
            ...
            if(!empty($child_ids)){
                $child_user = Admin::whereIn('parent_id',$child_ids)->get();
                if(!empty($child_user)){
                    $ids = [];
                    foreach($child_user as $chi){
                        $ids[] = $chi->id;
...
public static function getParentIds($role_id,$user_id,$i){
    ...
    if($i == 1){
        $distributor = Admin::where('parent_id',$user_id)->get();
        if(!empty($distributor)){
            $distributor_ids = [];
            foreach($distributor as $all){
                $distributor_ids[] = $all->id;
            }
            if(!empty($distributor_ids)){
                $tsms = Admin::whereIn('parent_id',$distributor_ids)->get();
                if(!empty($tsms)){
                    $tsm_ids = [];
                    foreach($tsms as $all){
                        $tsm_ids[] = $all->id;
            ...
            if(!empty($tsm_ids)){
             $sellers = Admin::whereIn('parent_id',$tsm_ids)->get();
             if(!empty($sellers)){
                $seller_ids = [];
                foreach($sellers as $all){
                    $seller_ids[] = $all->id;
                }
            }
...
if($i == 2){
    $tsms = Admin::where('parent_id',$user_id)->get();
    if(!empty($tsms)){
        $tsm_ids = [];
        foreach($tsms as $all){
            $tsm_ids[] = $all->id;
...
if($i == 3){
    $sellers = Admin::where('parent_id',$user_id)->get();
    if(!empty($sellers)){
        $seller_ids = [];
        foreach($sellers as $all){
            $seller_ids[] = $all->id;
...
if($i == 4){
    $sellers = Admin::where('id',$user_id)->get();
    if(!empty($sellers)){
        $seller_ids = [];
        foreach($sellers as $all){
            $seller_ids[] = $all->id;
...
// public static function getAdminDetails($user_id,$role_id,$select_coloumn){
//     $admin = Admin::where('id',$user_id)->where('role_id',$role_id)->first();
...
// public static function getParentRoleName($role_id){
//     if($role_id !=0){
//         $role = Roles::where('id',$role_id)->first();
//         return $role->name??'';
//     }else{
...
public static function getUserDetailsById($user_id){
    if($user_id !=0){
        ...
        if(empty($business_name)){
            $business_name = $admins->name??'';
        }
        $html.=$business_name??'';
        $html.='<br>';
        $html.=$admins->unique_id??'';
...
public static function getChildRolesAll($role_id){
    $roles = Roles::where('parent_id','!=',0)->where('id','>',$role_id)->get();
    return $roles;   
}

public static function getNoOfChild($role_id){
    if($role_id !=0){
        $counts = Admin::where('parent_id',$role_id)->count();
        return $counts;
    }else{
        return 0;
    }    
...
public static function getUserDetailsSingle($user_id,$role_id){
    if($user_id !=0){
        $admins = Admin::where('id',$user_id)->where('role_id',$role_id)->first();
        return $admins;
    }else{
        return "";
    }    
...
public static function isFacultyAllowedModule($moduleName){
   // prd($moduleName);
    $isAllowed = false;
    $allowedModulesArr = config('modules.faculty');
    $moduleNameArrAnd = [];
    $moduleNameArrOr = [];
...
// echo 'here'; die;
    ...
    public static function sendAllUsers($request,$image){
       User::latest('id')->select('id', 'name')->where(['status'=>1,'role_id'=>2])->where('is_delete',0)
       ->chunk(50, function($users) use ($request,$image) {
                ...
                foreach ($user_logins as $key) {
                    ...
                    if(!empty($deviceToken)){
                        ...
                        if(!empty($image)){
                            ...
                            $imageUrl = url('public/storage/notification/notificationimg.png');
                        }
   ...
   public static function sendSingleUsers($request,$image){
       ...
       $user_logins = UserLogin::where(['user_id'=>$request->user_id,'role_id'=>2,'is_delete'=>0,'status'=>1])->get();
       if(!empty($user_logins)){
        foreach ($user_logins as $key) {
            $deviceToken = $key->deviceToken;
            if(!empty($deviceToken)){
...
public static function sendNotificationtoSeller($request,$image){

    $user_logins = Admin::where('role_id',5)->get();
    if(!empty($user_logins)){
        foreach ($user_logins as $key) {
            $deviceToken = $key->device_token??'';;
            if(!empty($deviceToken)){
                $title = $request->title1??'';
                $body = $request->text1??'';
                $imageUrl ='';
