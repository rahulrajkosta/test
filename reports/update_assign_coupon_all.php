...
$query = $db->query("SELECT id FROM `admins` WHERE `role_id` = ".$role." AND `status`='1' AND total_assign_coupon = NULL "); 
if($query->num_rows > 0){ 
    while($row = $query->fetch_assoc()){ 
        ...
        $db->query("UPDATE  admins SET `total_assign_coupon`=".$in_coupon." WHERE id = ".$row['id']."");
    } 
}