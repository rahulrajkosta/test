...
$fileName = "zero_touch_remove_data" . date('Y-m-d') . ".xls"; 

// Column names 
$fields = array('couponID','Super Dist Details', 'Distributor Details', 'Sales Person Details', 'Retailer Details', 'Super Distributor State', 'Distributor State','Sales Person State',"Retailer State",'Uses/UnUsed','User Name','User Phone','Mobile Name','Imei No','Date Of Purchase','App Status','Date'); 
$excelData = implode("\t", array_values($fields)) . "\n"; 
if(!empty($start_date) && !empty($end_date)){
$sql = "SELECT coupon_id,user_name,user_phone,imei_no,date_of_purchase,coupon_parent_id,phone_status,mobile_name FROM `mobile_details` WHERE date(`date_of_purchase`) >= '".$start_date."' AND date(`date_of_purchase`) <= '".$end_date."' AND `app_status`='remove' ";

}else{
    $sql = "SELECT coupon_id,user_name,user_phone,imei_no,date_of_purchase,coupon_parent_id,phone_status,mobile_name FROM `mobile_details` WHERE app_status='remove'  ";
}
...
if($query->num_rows > 0){ 
    while($row = $query->fetch_assoc()){ 
        ...
        $lineData = array($row['coupon_id'], $super_distributor_data, $distributor_data,$tsm_data, $seller_data, $super_distributor_state,$distributor_state,$tsm_state,$seller_state,"Used",$row['user_name'],$row['user_phone'],$row['mobile_name'],$row['imei_no'],$row['date_of_purchase'],$row['phone_status'],$row['date_of_purchase']); 
